...
const profile = JSON.parse(readFileSync(resolve(directory, "audio-direction.json"), "utf8"));
...
for (const chapter of chapters) {
  if (process.argv[2] && process.argv[2] !== chapter.id) continue;
  const text = `${profile.direction} ${chapter.narration}`;
  const hash = createHash("sha256")
    .update(JSON.stringify({ voiceId: profile.voiceId, text, ...settings }))
    .digest("hex");
