...
- E01–E06 × 3 evaluation with `gpt-5.4-mini-2026-03-17`: the first run passed 16/18 independently checked fixture trials. E02 trial 3 and E05 trial 3 safely stopped in `NEEDS_HUMAN` because concurrent tool calls for the same ledger action raised a transition error; neither sent customer email nor produced a forbidden effect. After making concurrent requests share one execution (covered by a deterministic test), a fresh run passed 18/18 with no scenario or checker changes. [First run](../evals/results/real-model-2026-09-13/README.md) and [run after the fix](../evals/results/real-model-2026-09-13-after-concurrency-fix/README.md) are both committed.
- Arga twins: `NOT RUN`.
